"""
pull_requests.py — Download merged PRs + review discussions from Linkerd repos.

Complementary to issues.py. PRs provide:
...
Usage:
    ...
    python scrapers/pull_requests.py
    python scrapers/pull_requests.py --repos linkerd/linkerd2 --output data/raw_prs.jsonl
...
def fetch_repo_prs(repo: str, headers: dict, output_path: Path) -> int:
    ...
    with open(output_path, "a") as f:
        for pr in paginate(url, headers, params):
            ...
            title_preview = pr["title"][:70]
            print(f"  [{count}] #{pr['number']}: {title_preview}", flush=True)
