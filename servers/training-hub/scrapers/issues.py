"""
issues.py — Download all issues + comments from Linkerd GitHub repos.

Usage:
    export GITHUB_TOKEN=ghp_...
    python scrapers/issues.py
    python scrapers/issues.py --repos linkerd/linkerd2 --output data/raw_issues.jsonl
    ...
    while True:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Handle rate-limit responses (403 / 429) before raise_for_status so we
                # can sleep until the window resets instead of crashing.
                if resp.status_code in (403, 429):
                    reset_ts = int(resp.headers.get("X-RateLimit-Reset", time.time() + 60))
                    wait = max(reset_ts - time.time() + 2, 5)
                    print(f"    Rate limited (HTTP {resp.status_code}) — sleeping {wait:.0f}s "
                          f"then retrying (attempt {attempt + 1}/{max_retries}) …", flush=True)
                    time.sleep(wait)
                    continue
...
def fetch_repo_issues(repo: str, headers: dict, output_path: Path) -> int:
    ...
    with open(output_path, "a") as f:
        for issue in paginate(url, headers, params):
            ...
            title_preview = issue["title"][:70]
            print(f"  [{count}] #{issue['number']}: {title_preview}", flush=True)
